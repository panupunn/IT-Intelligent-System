...
with tab_user:
    ...
    if up:
        ...
        if err: st.error(err)
            ...
            # รองรับหัวตาราง: Username, DisplayName, Role, Active, Password, PasswordHash
...
def page_issue_receive(sh):
    ...
    if st.session_state.get("role") not in ("admin","staff"): st.info("สิทธิ์ผู้ชมไม่สามารถบันทึกรายการได้"); 
with tab_user:
    import pandas as pd, bcrypt
    st.markdown("##### อัปโหลดไฟล์")
    up = st.file_uploader("อัปโหลดไฟล์ ผู้ใช้ (CSV/Excel)", type=["csv","xlsx"], key="up_users")
    ...
    if items.empty: st.warning("ยังไม่มีรายการอุปกรณ์ในคลัง"); 
with tab_user:
    import pandas as pd, bcrypt
    st.markdown("##### อัปโหลดไฟล์")
    up = st.file_uploader("อัปโหลดไฟล์ ผู้ใช้ (CSV/Excel)", type=["csv","xlsx"], key="up_users")
...
with tab_user:
    ...
    st.markdown("</div>", unsafe_allow_html=True); return
        ...
        if row.empty:
            st.error("ไม่พบผู้ใช้ปัจจุบันในชีต Users"); 
with tab_user:
    import pandas as pd, bcrypt
    st.markdown("##### อัปโหลดไฟล์")
    up = st.file_uploader("อัปโหลดไฟล์ ผู้ใช้ (CSV/Excel)", type=["csv","xlsx"], key="up_users")
...
with tab_user:
    ...
    st.markdown("</div>", unsafe_allow_html=True); return
            ...
            st.error("ไม่สามารถตรวจสอบรหัสผ่านได้"); 
with tab_user:
    import pandas as pd, bcrypt
    st.markdown("##### อัปโหลดไฟล์")
    up = st.file_uploader("อัปโหลดไฟล์ ผู้ใช้ (CSV/Excel)", type=["csv","xlsx"], key="up_users")
    ...
    st.markdown("</div>", unsafe_allow_html=True); return
        if confirm.strip().upper() != "CLEAR":
            st.error("กรุณาพิมพ์คำว่า CLEAR ให้ถูกต้องเพื่อยืนยัน"); 
with tab_user:
    import pandas as pd, bcrypt
    st.markdown("##### อัปโหลดไฟล์")
    up = st.file_uploader("อัปโหลดไฟล์ ผู้ใช้ (CSV/Excel)", type=["csv","xlsx"], key="up_users")
